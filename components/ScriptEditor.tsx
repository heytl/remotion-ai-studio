...
export const ScriptEditor: React.FC<{ project: Project; patch: Patch; onNext: () => void }> = ({ project, patch, onNext }) => {
  ...
  const generateTts = async (index: number) => { const scene = scenes[index]; if (!scene.narration.trim()) { setError('该场景没有旁白文本，无法生成配音'); return; } setTtsIndex(index); setError(null); try { const data = await apiPost<{ audioDataUrl: string }>('/api/tts', { text: scene.narration }); updateScene(index, { audioDataUrl: data.audioDataUrl }); } catch (e) { setError((e as Error).message); } finally { setTtsIndex(null); } };
  const generateAllTts = async () => { setTtsAll(true); setError(null); try { const nextScenes = [...scenes]; for (let index = 0; index < scenes.length; index++) { if (!scenes[index].narration.trim()) continue; setTtsIndex(index); const data = await apiPost<{ audioDataUrl: string }>('/api/tts', { text: scenes[index].narration }); nextScenes[index] = { ...nextScenes[index], audioDataUrl: data.audioDataUrl }; patch({ script: [...nextScenes] }); } } catch (e) { setError((e as Error).message); } finally { setTtsIndex(null); setTtsAll(false); } };
            ...
            <CardContent className="p-4 sm:p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1 space-y-4">
                  ...
                  <Field label="旁白 / 字幕"><Textarea value={scene.narration} onChange={(e) => updateScene(index, { narration: e.target.value })} /></Field>
