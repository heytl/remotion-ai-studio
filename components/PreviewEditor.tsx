...
import { ArrowRight, FilmSlate, MonitorPlay, Palette, SlidersHorizontal } from '@phosphor-icons/react';
...
export const PreviewEditor: React.FC<{ project: Project; patch: Patch; onNext: () => void }> = ({ project, patch, onNext }) => {
  ...
  const updateTheme = (partial: Partial<typeof schema.theme>) => patch({ schema: { ...schema, theme: { ...schema.theme, ...partial } } });
  const updateScene = (index: number, partial: Partial<VideoScene>) => { const scenes = schema.scenes.slice(); scenes[index] = { ...scenes[index], ...partial }; patch({ schema: { ...schema, scenes } }); };
  ...
  return (
    <div className="space-y-6">
      <PageHeading eyebrow="Step 04 · Visual studio" title="Remotion 实时预览" description={`共 ${schema.scenes.length} 个场景 · ${formatSeconds(totalSeconds)}，调整参数后播放器会实时更新。`} actions={<Button onClick={onNext}>进入导出<ArrowRight className="h-4 w-4" weight="bold" /></Button>} />
        ...
        <Card className="h-fit xl:sticky xl:top-[90px] xl:max-h-[calc(100vh-110px)] xl:overflow-y-auto">
          ...
          <CardContent className="space-y-6 pt-5">
            ...
            {scene && <section className="space-y-4 border-t border-border/15 pt-5" aria-labelledby="scene-settings"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><FilmSlate className="h-4 w-4 text-cyan-200" weight="fill" /><h3 id="scene-settings" className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">场景 {active + 1}</h3></div><Badge variant="outline">{formatSeconds(scene.durationSeconds)}</Badge></div>
              ...
              <Field label="旁白 / 字幕"><Textarea className="min-h-20" value={scene.narration} onChange={(e) => updateScene(active, { narration: e.target.value })} /></Field>
              <Field label="要点（每行一条）"><Textarea className="min-h-20" value={toLines(scene.bullets)} onChange={(e) => updateScene(active, { bullets: parseLines(e.target.value) })} /></Field>
